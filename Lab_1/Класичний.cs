...
namespace MainCode
{
    ...
    {
        private const int FileSizeMB = 20;  // Припустимо, що розмір файлу 20 MB
        ...
        private static void MultiPhaseSortFile(string filePath, int m)
        {
            ...
            string[] tempFiles = new string[m]; // m файлів
            for (int i = 0; i < m; i++)
            {
                tempFiles[i] = $"temp_chunk_{i}.txt";
            }

            // 1. Розподіляємо серії на основі чисел Фібоначчі порядку m-2
        ...
        private static List<string> SplitAndSortChunks(string filePath, string[] tempFiles, int chunkSize, int m)
        {
            ...
            using (StreamReader reader = new StreamReader(filePath))
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    // Переходимо до наступного файлу, якщо поточний завершив
